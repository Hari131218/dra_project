...
class _BasicState extends State<Basic> {
  ...
  fetchData(accessToken) async {
    ...
    final response = await get(Uri.parse('${BASE_URL}assessments/show?id=${widget.ids}'),headers:
    {'Authorization':
    'Bearer ${accessToken}'});
    print(response.body);
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        DateTime parseDate =
        new DateFormat("MM/dd/yyyy").parse(date);
  ...
  rescheduleAPICall(_dat) async {
    ...
    final response = await post(
        Uri.parse(
            '${BASE_URL}assessment_requests/reschedule_assessment'), body: {
                    ...
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>AssesmentPage(str_id: '${widget.ids}',)));
                          },
                          child: Center(
                            child: Text(
